...
function xalloc(xpath: XPath[], opt: Opt) {
    ...
    xpath.forEach(x => {
        ...
        if (base.startsWith(opt.at!)) {
            ...
            attrifneed(current, base.substring(opt.at!.length), escape(x.value), opt);
...
function attrifneed(e: ElementCompact, attr: string, value: string, opt: Opt) {
    if (opt.autofilter?.spec === attr) {
        // omit attribute which is generated by autofilter
        return;
    }
